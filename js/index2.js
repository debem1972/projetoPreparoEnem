...
async function importData(dataOrFile) {
    try {
        ...
        // Limpar stores existentes apenas se for uma importação remota
        if (!(dataOrFile instanceof File)) {
...
async function checkAndUpdatePlan() {
    try {
        ...
        if (!remoteData) {
            // Se não conseguiu carregar o JSON remoto, carrega os dados locais
            const savedState = localStorage.getItem('planState');
            if (savedState) {
                const { month, week, day } = JSON.parse(savedState);
        ...
        // Comparar datas e importar se necessário
        if (lastRemoteUpdate > lastLocalUpdate) {
            console.log('Nova versão do plano detectada. Importando...');
